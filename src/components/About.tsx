function About() {
  return (
    <section id="about" className="min-h-screen flex items-center px-6 py-24">
      <div className="max-w-5xl mx-auto">
        <h2 className="font-serif text-5xl md:text-6xl text-white mb-8 tracking-wide text-center">
          About me
        </h2>

        <div className="h-px w-24 mx-auto bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent mb-16"></div>

        <div className="space-y-8 text-white/80 text-lg font-light leading-relaxed tracking-wide">
          <p>
            I'm the Director of Public Relations for the Los Santos City Government, working with the people of Los Santos and maintaining the government's reputation. I have a Bachelor of Fine Arts, with experience in photography and graphic design, which I use to make city visually engaging, and professional.
          </p>

          <p>
            My goal is to help the city look visually beautiful and to connect people through clear and engaging communication. I strive to make every project reflect the city's uniqueness while bringing the community closer together.
          </p>

          <p>
            
          </p>
        </div>
      </div>
    </section>
  );
}

export default About;
